...
export function buildRecentFormEvidence(ctx: MatchContext): Evidence[] {
  ...
  const process = (side: 'home' | 'away') => {
    ...
    if (winStreak >= 2) {
      out.push({ id: uid('rf-wst-'), category: EvidenceCategory.RECENT_FORM, importance: EvidenceImportance.HIGH, confidence: scoreConfidence(played, 5), description: `${side} on a ${winStreak}-match winning streak.`, details: { winStreak } })
    }
    if (unbeatenStreak >= 3) {
      out.push({ id: uid('rf-ust-'), category: EvidenceCategory.RECENT_FORM, importance: EvidenceImportance.HIGH, confidence: scoreConfidence(played, 5), description: `${side} unbeaten in the last ${unbeatenStreak} matches.`, details: { unbeatenStreak } })
    }

    // 3) attacking/defensive averages
    out.push({ id: uid('rf-avg-'), category: EvidenceCategory.RECENT_FORM, importance: scoreImportance(avgFor, 'goals'), confidence: scoreConfidence(played, 5), description: `${side} averaging ${avgFor} goals for and ${avgAgainst} goals against per match.`, details: { avgFor, avgAgainst } })

    // 4) clean sheets / failed to score
    if (cleanSheets >= 2) {
      out.push({ id: uid('rf-cs-'), category: EvidenceCategory.RECENT_FORM, importance: EvidenceImportance.MEDIUM, confidence: scoreConfidence(played, 5), description: `${side} kept ${cleanSheets} clean sheets in recent form.`, details: { cleanSheets } })
    }
    if (failedToScore >= 2) {
      out.push({ id: uid('rf-fs-'), category: EvidenceCategory.RECENT_FORM, importance: EvidenceImportance.MEDIUM, confidence: scoreConfidence(played, 5), description: `${side} failed to score in ${failedToScore} of recent matches.`, details: { failedToScore } })
    }
  }
...
export function buildHeadToHeadEvidence(ctx: MatchContext): Evidence[] {
  ...
  if (homeUnbeaten && count >= 3) {
    out.push({ id: uid('h2h-unb-'), category: EvidenceCategory.HEAD_TO_HEAD, importance: EvidenceImportance.HIGH, confidence: scoreConfidence(count, 5), description: `Home team unbeaten in last ${count} meetings.`, details: { homeUnbeaten, count } })
  }

  if (bttsRatio >= 0.6) {
    out.push({ id: uid('h2h-btts-'), category: EvidenceCategory.HEAD_TO_HEAD, importance: EvidenceImportance.HIGH, confidence: scoreConfidence(count, 5), description: `Both teams scored in ${Math.round(bttsRatio * 100)}% of recent meetings.`, details: { bttsRatio } })
  } else if (bttsRatio > 0) {
    out.push({ id: uid('h2h-btts-low-'), category: EvidenceCategory.HEAD_TO_HEAD, importance: EvidenceImportance.MEDIUM, confidence: scoreConfidence(count, 5), description: `BTTS occurred in ${Math.round(bttsRatio * 100)}% of recent head-to-head matches.`, details: { bttsRatio } })
  }

  if (over25Ratio >= 0.6) {
    out.push({ id: uid('h2h-over-'), category: EvidenceCategory.HEAD_TO_HEAD, importance: EvidenceImportance.MEDIUM, confidence: scoreConfidence(count, 5), description: `Over 2.5 goals in ${Math.round(over25Ratio * 100)}% of recent meetings.`, details: { over25Ratio } })
  }

  if (avgGoals > 2.5) {
    out.push({ id: uid('h2h-avg-'), category: EvidenceCategory.HEAD_TO_HEAD, importance: EvidenceImportance.MEDIUM, confidence: scoreConfidence(count, 5), description: `Average goals across last ${count} meetings: ${avgGoals.toFixed(1)}.`, details: { avgGoals } })
  }
...
export function buildLeagueEvidence(ctx: MatchContext): Evidence[] {
  ...
  const processSide = (side: 'home' | 'away') => {
    ...
    if (gapLeader !== undefined && gapLeader <= 3) {
      out.push({ id: uid('lg-gap-'), category: EvidenceCategory.LEAGUE, importance: EvidenceImportance.CRITICAL, confidence: scoreConfidence(1, 1), description: `${side} are only ${gapLeader} points behind the leader.`, details: { gapLeader } })
    } else if (gapLeader !== undefined && gapLeader <= 8) {
      out.push({ id: uid('lg-gap-med-'), category: EvidenceCategory.LEAGUE, importance: EvidenceImportance.HIGH, confidence: scoreConfidence(1, 1), description: `${side} are ${gapLeader} points off the top.`, details: { gapLeader } })
    }

    if (gapReleg !== undefined && gapReleg <= 3) {
      out.push({ id: uid('lg-releg-'), category: EvidenceCategory.LEAGUE, importance: EvidenceImportance.CRITICAL, confidence: scoreConfidence(1, 1), description: `${side} are ${gapReleg} points above relegation.`, details: { gapReleg } })
    }
...
export function buildLineupEvidence(ctx: MatchContext): Evidence[] {
  ...
  const detectMissing = (side: 'home' | 'away') => {
    ...
    if (Array.isArray(info.starters) && info.starters.length && info.starters.length < 11) {
      out.push({ id: uid('ln-smol-'), category: EvidenceCategory.LINEUP, importance: EvidenceImportance.MEDIUM, confidence: 0.8, description: `${side} starting XI incomplete (${info.starters.length} players).`, details: { startersCount: info.starters.length } })
    }
  }
